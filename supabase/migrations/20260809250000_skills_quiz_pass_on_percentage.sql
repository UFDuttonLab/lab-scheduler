-- =====================================================================================
-- 2026-08-09  Skills quiz: pass on the overall percentage alone.
--
-- APPLIED LIVE to ypaobygipbnkvnismhyy on 2026-08-09.
--
-- The original rule was "100% of critical questions AND >= quiz_pass_pct overall". A review
-- of the finished system showed what that actually meant in practice: every skill carries
-- 5-7 questions, so 80% already tolerates at most ONE wrong answer (6/7 = 86% passes, 5/7 =
-- 71% fails), and 167 of the 353 questions are flagged critical. The critical veto therefore
-- almost never changed the outcome except to fail someone at 86% for a single slip, and it
-- made every one of those 167 answer keys load-bearing: one wrong key would have locked that
-- skill for the whole lab, unfixable by retaking, with a per-person PI waiver as the only
-- route through.
--
-- Chris's decision: keep the bar at 80% flat, regardless of critical.
--
-- Critical questions are NOT going away. They are still flagged in the bank, still shown to
-- the trainee, still counted per attempt (n_critical / n_critical_correct on every row of
-- skill_quiz_attempts), so "passed but missed a critical one" stays visible and reviewable.
-- Only the veto is removed.
--
-- Nothing else in the function changes: the DISTINCT ON de-duplication that closed the
-- score-inflation exploit, the reveal-only-on-pass rule, the unknown-id rejection and the
-- keyless-question tolerance are all carried through unmodified.
-- =====================================================================================
