-- Update log_activity function to handle tables without user_id column
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME IN ('projects', 'equipment', 'app_versions') THEN
    -- For tables without user_id column, just use auth.uid()
    v_user_id := auth.uid();
  ELSE
